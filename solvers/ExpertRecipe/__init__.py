...
def Get_Quality_AllowSkills(craft: Craft.Craft, craft_history: list = []) -> set:
    ...
    if now_dur <= 30 + craft.get_skill_durability("加工"): forbidden_actions.add("加工") # 耐久不足
    if now_dur <= 10 + craft.get_skill_durability("俭约加工"): forbidden_actions.add("俭约加工") # 耐久不足
    ...
    if craft.current_cp < 81: # [-俭约加工-阔步-比尔格]**CP不足
        forbidden_actions.add("俭约加工") 
        forbidden_actions.add("观察") 
    if craft.current_cp < 42 + craft.get_skill_cost("阔步") and "改革" not in craft.effects:#[阔步-改革-比尔格]**CP不够
        forbidden_actions.add("阔步")
    if craft.current_cp < 56 + craft.get_skill_cost("改革") and "阔步" not in craft.effects:#[阔步-改革-比尔格]**CP不够
        forbidden_actions.add("改革")
    if craft.current_cp < 24 + craft.get_skill_cost("阔步"): # [-阔步-比尔格]**CP不足
        forbidden_actions.add("工匠的神技")
        forbidden_actions.add("阔步")
    if craft.current_cp < 24 + craft.get_skill_cost("改革"): forbidden_actions.add("改革") # [-改革-比尔格]**CP不足
    result_actions = set()
