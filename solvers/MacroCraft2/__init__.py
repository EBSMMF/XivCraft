...
from .MacroParser import Macro, MacroFinish
...
class MacroContainer(object):
    def __init__(self, macro_str, file_name):
        ...
        self.min_craftmanship, self.min_control, self.min_cp = map(int, (self.tags["MinAttr"] if "MinAttr" in self.tags else "0/0/0").split('/'))
        self.max_craftmanship = int(self.tags["MaxCraft"]) if "MaxCraft" in self.tags else -1
        self.ignore_quality = "IgnoreQuality" in self.tags

    def attr_pair(self, player):
        if 0 < self.max_craftmanship < player.craftmanship or player.craftmanship < self.min_craftmanship:
            return False
